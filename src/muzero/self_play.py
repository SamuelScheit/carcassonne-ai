...
class SelfPlay:
        ...
        while ray.get(shared_storage.get_info.remote(
            ...
            if not test_mode:
                ...
                shared_storage.set_info.remote({
                    "episode_length":
                    len(game_history.action_history) - 1,
                    "total_reward":
                ...
                if 1 < len(self.config.players):
                    shared_storage.set_info.remote({
                        "muzero_reward":
                        sum(reward for i, reward in enumerate(
    ...
    def play_game(self, temperature, temperature_threshold, render, opponent,
                  muzero_player):
        """
        ...
        with torch.no_grad():
            while (not done and
                   len(game_history.action_history) <= self.config.max_moves):
                assert (
    ...
    def ucb_score(self, parent, child, min_max_stats):
        ...
        if child.visit_count > 0:
            ...
            value_score = min_max_stats.normalize(
                child.reward + self.config.discount *
                (child.value() if len(self.config.players) ==
                 1 else -child.value()))
        else:
            value_score = 0
    ...
    def backpropagate(self, search_path, value, to_play, min_max_stats):
        ...
        if len(self.config.players) == 1:
            for node in reversed(search_path):
                ...
                node.visit_count += 1
                min_max_stats.update(node.reward +
                                     self.config.discount * node.value())
            ...
            for node in reversed(search_path):
                ...
                value = (-node.reward if node.to_play == to_play else
                         node.reward) + self.config.discount * value
...
class Node:
    ...
    def expand(self, actions, to_play, reward, policy_logits, hidden_state):
        ...
        policy_values = torch.softmax(torch.tensor(
    ...
    def add_exploration_noise(self, dirichlet_alpha, exploration_fraction):
        ...
        for a, n in zip(actions, noise):
            self.children[a].prior = self.children[a].prior * (1 -
                                                               frac) + n * frac

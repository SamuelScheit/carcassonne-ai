...
from carcassonne2 import Game, MuZeroConfig
...
class MuZero:
    ...
    def train(self, log_in_tensorboard=True):
        ...
        if self.config.use_last_model_value:
            self.reanalyse_worker = replay_buffer.Reanalyse.options(
                ...
                if self.config.reanalyse_on_gpu else 0,
            ).remote(self.checkpoint, self.config)
        ...
        self.self_play_workers = [
            self_play.SelfPlay.options(
                num_cpus=0,
                num_gpus=num_gpus_per_worker
                if self.config.selfplay_on_gpu else 0,
            ).remote(
                self.checkpoint,
                self.Game,
                self.config,
        ...
        if self.config.use_last_model_value:
            self.reanalyse_worker.reanalyse.remote(self.replay_buffer_worker,
    ...
    def logging_loop(self, num_gpus):
        ...
        try:
            while info["training_step"] < self.config.training_steps:
                ...
                writer.add_scalar("2.Workers/2.Training_steps",
                                  info["training_step"], counter)
                writer.add_scalar("2.Workers/3.Self_played_steps",
                                  info["num_played_steps"], counter)
                writer.add_scalar(
                ...
                writer.add_scalar("2.Workers/6.Learning_rate", info["lr"],
                                  counter)
                writer.add_scalar("3.Loss/1.Total_weighted_loss",
                                  info["total_loss"], counter)
                writer.add_scalar("3.Loss/Value_loss", info["value_loss"],
                                  counter)
                writer.add_scalar("3.Loss/Reward_loss", info["reward_loss"],
                                  counter)
            ...
            pickle.dump(
                {
                    ...
                    "num_played_steps":
                    self.checkpoint["num_played_steps"],
                    "num_reanalysed_games":
    ...
    def test(self,
        ...
        self_play_worker = self_play.SelfPlay.options(
            num_cpus=0,
            num_gpus=num_gpus,
        ).remote(self.checkpoint, self.Game, self.config,
        ...
        if len(self.config.players) == 1:
            ...
            result = numpy.mean([
                sum(reward for i, reward in enumerate(history.reward_history)
...
def hyperparameter_search(game_name, parametrization, budget,
    ...
    if best_training:
        ...
        torch.save(
            ...
            os.path.join(best_training["config"].results_path,
                         "model.checkpoint"),
        )
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        # games = [
        #     filename[:-3] for filename in sorted(
        #         os.listdir(
        ...
        while True:
            ...
            if choice == 0:
                ...
                parametrization = nevergrad.p.Dict(lr_init=lr_init,
                                                   discount=discount)
